const response = 'fraiondeeyeccion',
	...
	$('#open-enviar').on('click', function () {
		if (valdiar()) {
			...
			if (response === validateWord()) {
				...
				if (respuestas.find((item) => item === 'slider125')) {
					respuestas.push('slider125')
				...
				chuloSound.play()
				setTimeout(() => {
					window.location.href = './index126.html'
				}, 3000)
			} else {
				localStorage.setItem('error', 9)
